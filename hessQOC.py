...
def mycb(xk, status=None):
    ...
    overallcost = controlcost + rho*residnorm2
    normgrad = np.linalg.norm(gradobj(xk))
    thesestats = np.array([residnorm2, controlcost, overallcost, normgrad])
    f = open(outpath+'livestats_'+mol+'_'+basis+postfix+'.txt','a')
    xx = "0.5*||resid||^2 = " + str(residnorm2) + "  0.5*||f||^2 = " + str(controlcost) + "  overallcost = " + str(overallcost) + "  ||grad|| = " + str(normgrad)
    f.write(xx)
    f.write("\n")
    f.close()
    return False
